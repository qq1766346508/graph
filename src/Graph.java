...
public class Graph {
	...
	public Graph(int size) {
		...
		edgeCount = 0;
		edgeweight = new int[100]; // 先用100条边
	}
		...
		while (!stack.isEmpty()) {
			...
			while (p != null) {
				...
				if (p != null) {
					...
					/**
					 * 判断新得到的事件最早发生时间是否比原先的事件最早发生时间长 公式：ve[1] = 0; ve[k] = max{ve[j]+len<vj,vk>}
					 */
	...
	public void calculate() {
		...
		/**
		 * 按逆拓扑有序来求其余各顶点的最迟发生时间 原理：restack里保存着拓扑排序的顺序数列，将从restack里将最后一个节点取出，即没有事件依赖于它的结点
		 * 取出的数放进backstack中,
		 * 再从restack里一个一个取出节点元素，看其是否跟已放入backstack中的元素相连，若有相连，则进行vl的计算，如此重复 计算公式：vl[k] =
		 * min{vl[j]-len<vk,vj>}
		 */
		...
		while (!restack.isEmpty()) {
			...
			for (int k = 0; k < backstack.getCount(); k++) {
				...
				while (ver.getNext() != null) {
					 ...
					 */
					if (((Integer) ver.getNext().getData()).intValue() == backstack.getElement(k)) {
						int yuanxian = vl[((Integer) vertex.getData()).intValue()];
						int jiangyao = vl[backstack.getElement(k)] - ver.getNext().getWeight();
						/**
						 * 若两点之间相连的话，更新结点的vl值 计算vl的公式是：发vl[n] = ve[n] vl[k] = min{vl[j]-len<vk,vj>}
						 */
						if (jiangyao < yuanxian || yuanxian == -1) {
							vl[((Integer) vertex.getData()).intValue()] = vl[backstack.getElement(k)]
		...
		/**
		 * 求活动的最早开始时间e[i]与活动的最晚开始时间l[i] 若活动ai是由弧<Vk,Vj>表示，根据AOE网的性质，只有事件vk发生了，活动ai才能开始
		 * 。也就是说，活动ai的最早开始时间应等于事件vk的最早发生时间。 因此有e[i] = ve[k]; 活动ai的最晚开始时间是指，在不推迟整个工期的前提下
		 * ，ai必须开始的最晚开始时间。若ai由有向边<vi,vj>表示，则ai的最晚开始时间要保证事件vj的最迟发生时间不拖后 因此，应该有l[i] =
		 * vl[j] - len<vk,vj>
		 */
		...
		for (int h = 0; h < vertexNum; h++) {
			...
			if (begin != null) {
				...
				while (begin.getNext() != null) {
					...
					l[t++] = vl[((Integer) begin.getNext().getData()).intValue()] - begin.getNext().getWeight();
					begin = begin.getNext();
				}
